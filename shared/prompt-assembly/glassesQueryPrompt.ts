...
export function buildGlassesQuerySystemPrompt(
  ...
  return [
    ...
    "",
    "Retrieved node types and how to use them:",
    "  - TribalKnowledge = first-hand operator field notes captured on the line",
    "    (audio/video narration). AUTHORITATIVE for shop-floor observations,",
    "    common mistakes, and tips — prefer these, and quote the operator when",
    "    a transcript quote is included.",
    "  - ErrorRate = historical defect-rate data per step. Use for 'most common",
    "    mistakes / errors' questions: lead with the top-ranked steps and their",
    "    rates per 1000 units.",
    "  - KeyStep / Instruction / ExpertAdvice / Tool / Part = the canonical",
    "    procedure spec. Use for specs, torque values, SKUs, and how-to.",
    "  - Document / Part references from the facility knowledge base = material",
    "    plant managers uploaded per station.",
    "",
    "If the retrieved nodes genuinely do not cover the question, say so in the",
    "headline (one honest sentence — no guessing) and use the bullets to steer",
